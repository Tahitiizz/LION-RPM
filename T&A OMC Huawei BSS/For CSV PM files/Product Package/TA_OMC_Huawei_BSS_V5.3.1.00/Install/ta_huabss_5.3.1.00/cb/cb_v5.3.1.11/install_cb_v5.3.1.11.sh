...
if [ $user = 'root' ]; then
    ...
    # 07/04/2011 SCT : ajout du port postgresql
    # 11/04/2011 SCT : le port Postgresql passe en 8eme parametre pour la compatibilite
    # 12/04/2011 SCT : amelioration de la gestion des options IP et postgresql Port
    #postgresPort=$8     # port postgresql
    #if [ -z "$postgresPort" ] ; then
    #    postgresPort=$postgresDefaultPort
    #fi
	# 04/03/2011 SCT : Ajout d'un huitieme parametre optionnel : l'IP du serveur
    # 11/04/2011 SCT : l'IP reprend sa position d'origine (7eme parametre)
    #preset_ip=$7		# preset IP
    # 12/04/2011 SCT : nouvelle attribution des parametres IP et postgresql port
    option1=$7
    option2=$8
    messageLog=""
    # verification de l'existance d'une ip parmi les 2 options
    regex="\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b"
    if [ ! -z "$option1" ]; then
        # cas de l'option 1 qui est une ip
        checkIp=$(echo $option1 | egrep $regex)
        if [[ "$?" -eq 0 ]] ; then
            preset_ip=$option1
            messageLog=$messageLog"7th parameter is IP : "$option1"\n"
        # l'option1 est un entier
        elif [ "$(echo $option1 | grep "^[ [:digit:] ]*$")" ] && [ $option1 -lt 65535 ] ; then
    ...
    if [ ! -z "$option2" ]; then
        # cas de l'option 1 qui est une ip
        checkIp=$(echo $option2 | egrep $regex)
        if [[ "$?" -eq 0 ]] && [ -z "$preset_ip" ] ; then
            preset_ip=$option2
            messageLog=$messageLog"8th parameter is IP : "$option2"\n"
        # l'option1 est un entier
        elif [ "$(echo $option2 | grep "^[ [:digit:] ]*$")" ] && [ $option2 -lt 65535 ] && [ -z "$postgresPort" ] ; then
            postgresPort=$option2
            messageLog=$messageLog"8th parameter is PostgresqlPort : "$option2"\n"
        fi
    fi
    # dans le cas ou le port postgresql est erronne ou non renseigne
    if [ -z "$postgresPort" ] ; then
        postgresPort=$postgresDefaultPort
        messageLog=$messageLog"PostgresqlPort is not in install_cb parameters => defaut port is used : "$postgresDefaultPort"\n"
    fi
	...
	if [ ! -f "$java_dir/java" ] ; then
		...
		# TESTING OLD BINARY
		if [ ! -f "/opt/jdk1.5.0_02/bin/java" ] ; then
    ...
    if [ ! -z $preset_ip ] ; then
        ...
        server_ip_address="$(/sbin/ifconfig | grep "dr" | grep "Bcast" | grep -v "127.0.0.1" | head -1 | grep -Eo 'dr:([0-9]{1,3}\.){3}[0-9]{1,3}' | cut -d: -f2)"
	fi
    ...
    # 15/04/2011 SCT : check if postgresql port is valid
    # 11/05/2011 SCT : desaction de la fonctionnalite pour l'instant => CB non compatible avec la gestion des ports postgresql
    #resultTestConnectionPostgresql=$($psql_dir/psql -U postgres -p $postgresPort -d template1 -tAc "SELECT pg_database_size('template1');" 2>&1) >> /dev/null
    # Check resultat is integer (if not : postgresql port is asked to user)
    #connectionState=0
    #if [ "$(echo $resultTestConnectionPostgresql | grep "^[ [:digit:] ]*$")" ] ; then
    #    connectionState=1
    #    messageLog=$messageLog"Connection to postgresql is valid (port: "$postgresPort")\n"
    #else
    #    echo "CANNOT CONNECT TO DATABASE, PLEASE CHECK POSTGRESQL STATUS OR POSTGRESQL PORT !";
    #    exit 127
    #fi
    # FIN 11/05/2011 SCT
    # FIN 15/04/2011 SCT
	# IS DATABASE ALREADY EXISTS ?
	...
	# LAUNCHING INSTALLATION ####################################################################################
       ...
       # TESTING DOWNGRADE
       #if [ $enable_downgrade == "0" ] ; then
       #       isVersionCompatible=`php -r "echo version_compare('$current_version','$version_cb');"`
       #       if [ $isVersionCompatible == "1" ] ; then
       #              echo "Error : cannot downgrade current version"
       #              echo "Error : cannot downgrade current version" >> $rep_install'/SQL/'$file_install_log 2>&1
