 ...
 *      - erreurs js lors de la création du menu contextuel boutton droit sur dashboard
...
/**
 *
	- maj 18/02/2009, benoit : création d'une nouvelle fonction 'getDocumentElement()' et utilisation de celle-ci pour récupérer des valeurs
	 du document. En effet, les valeurs des élements de document.body retournées précedemment étaient incorrectes car l'application utilise
	 maintenant une DTD qui inhibe ces valeurs. Il faut dans ce cas utiliser document.documentElement. La fonction JS crée permet de détecter
...
function popGTMwithLinks(titre, msg, AA, NE, xpertLaunchURL,xpertMenuLabel) {
	...
	//alert(xpertSupportedProductsArray);
...
function popWithEvent(evt,titre, msg, AA, NE) {
	...
	if ( (AA != '' && AA != undefined) || (NE != '' && NE != undefined) ) {
		...
		//12/01/2011 MMT DE Xpert pas de separateur si deja crée par Le menu Xpert
		if(!linkXpert){
			_myMenuContextuel.addMenuTemp({separator : true, id : 'separateurLinks'});
		}
      ...
      // 31/08/2010 MMT - DE firefox bz 17306 ajout du paramètre 'utilisation activeX oui/non'
      // pour liens AA. utilisation d'un nouveau delimiteur de valeur |j| du a la grande complexité de récuperer
...
// 31/08/2010 MMT - DE firefox bz 17306
// fonction destinée a savoir si le navigateur client peut supporter ActiveX (IE) et donc lancer AA directement
...
function trackMouse(evt) {
   // 31/08/2010 MMT - DE firefox bz 17306 gestion de l'evenement Prototype passé en paramètre
...
// 31/08/2010 MMT bz 17306:compatibilité Firefox utilisation parametre evt
...
/**
 * 05/07/2010 BBX
 * Réécriture de la classe pour correction de bug + compatibilités tous navigateurs
 * BZ 13754
 */
...
// (documentElement ou body suivant la présence / abscence d'une DTD)
...
document.observe("dom:loaded", function() {
    // Chaque élément survolé est mémorisé
    ...
    Event.observe(document, 'click', function(event) {
        if(_myTooltip != null) _myTooltip.hideTip();
        _myTooltip = null;
    });
});
...
/**
 * 05/07/2010 BBX
 * Réécriture de la fonction popalt. BZ 13754
 * Nouveautés :
...
function popalt(msg)
{
    // Si un élément est survolé
    if(_toolTipCallingElement != null)
    {
        // Si classe Tooltip échouée, utilisation d'un title
        if(_useOldTooltip) {
            _toolTipCallingElement.writeAttribute("title", msg);
            return false;
        }

        // Si Tooltip existe, on le détruit
        if(_myTooltip != null) {
            _myTooltip.hideTip();
            _myTooltip = null;
        }
