...
--
-- SCT 14:24 15/05/2009 Mise à jour des séquences pour éviter les problèmes
...
-- Désactivation de la step update_object_ref
UPDATE sys_definition_step SET on_off = 0 WHERE step_id = 118;

-- Ajout CCT1 04/11/08 table de paramétrage des produits.

-- 05/11/2008 - Modif. benoit : ajout du champ 'sdp_db_port' indiquant le port de la base de données du produit
...
-- 14/11/2008 BBX : ajout d'un fonction pl_sql permettant de calculer le chemin d'agrégation d'un NA
...
-- 26/11/2008 BBX : on renomme le paramètre de délai de l'autorefresh
...
-- 26/11/2008 BBX : Création de la table qui va permettre d'enregistrer des sélecteurs
...
---
-- 08/12/2008 BBX : création des 2 tables de jointure pour les DE sur les alarmes systèmes
---
...
-- Par défaut, tous les types de fichiers sont cochés pour toutes les connections
-- Par défaut les admin sont destinataires
...
-- Création d'une fonction psql pour initialiser les export data déjà crées
...
-- Paramètre de debug du mapping
DELETE FROM sys_debug WHERE parameters = 'mapping';
INSERT INTO sys_debug VALUES ('mapping', '0', 'mode debug du mapping de la topologie : 0 = désactivé / 1 = activé');


-- 14:35 18/12/2008 GHX
-- Création des index sur les tables des fichiers archivés. Elles permettent de gagnés du temps sur une requête principalement qui est exécute à la fin de la collecte des fichiers
...
-- Ajout des colonnes spécifiques au parser DTS Huawei (cela peut toujours servir)
...
-- Ajout d'une colonne pour gérer le séparateur spécifique d'un couple de NA du 1er axe
ALTER TABLE sys_definition_categorie DROP COLUMN separator;
ALTER TABLE sys_definition_categorie ADD COLUMN separator TEXT;

-- 22/01/2009 GHX requetes déplacé dans le fichier migrationGTMDashboardCB41.sql

-- 14/01/2009 - maj MPR : Ajout de la catégorie global parameters ( paramètre globaux du produit master => IHM) 
...
-- 14/01/2009 - maj MPR : Définition des paramètres globaux ( paramètres utilisés par le master pour l'IHM)
...
-- 14/01/2009 - maj MPR : Suppression des paramètres obselètes
...
-- 30/12/2008 GHX
-- Ajout d'une colonne pour avoir la possibilité d'utiliser les identifiants des NA au lieu de leurs labels
ALTER TABLE sys_export_raw_kpi_config ADD COLUMN use_code_na smallint default 0;

-- 23/01/2009 MPR - Ajout d'une colonne na_axe3 pour séparer le niveau d'agrégation 3ème axe du niveau d'agrégation réseau
...
-- 02/02/2009 - Modif. benoit : ajout de la colonne '3rd_axis_default_level' à la table 'sys_definition_network_agregation'
-- afin de gérer le cas de la sélection de plusieurs axes 3 différents dans un même GTM
...
-- On ne reference plus dans ce champ le chemin complet de l'image mais simplement le nom du style css à appliquer
...
-- 02/03/2009 - Modif. benoit : ajout du parametre 'dashboard_display_max_nb_queries_overtime' dans 'sys_global_parameters' permettant de définir le nombre maximal
-- de requetes executées afin de déterminer les NEs affichées dans les GTMs du dashboard
...
--
-- Création du la table qui contiendra la liste des tables qui pourront se trouver dans un contexte
--
-- sdct_id      => correspondra à l'identifiant unique à supprimer
...
--
-- Création du la table qui contiendra les clés primaires et étrangères des tables contenu dans la table sys_definition_context_tables
--
-- sdctk_id                => identifiant
-- sdctk_column       => nom de la colonne
-- sdctk_sdct_id      => identifiant [sys_definition_context_table.sdct_id] de la table  à laquelle fait référence la colonne
-- sdctk_sdctk_id    => si c'est une clé étrangère, identifiant [sdctk_id] de la colonne qui fait office de clé primaire. Si c'est une clé primaire, le champ est nul.
--
...
-- Modification du nom de la colonne de la clé étrangère
...
--
-- Création du la table qui contiendra les éléments qui peuvent être dans un contexte (graphe, dashboard, alarmes...)
--
-- sdc_id                                         => identifiant
-- sdc_label                                     => label de l'élément pour l'affichage
-- sdc_column                                 => nom de la colonne qui doit être affiché, pas besoin d'être configurer si la valeur sdc_selected est 0
--                                                           ATTENTION : la colonne est celle de la première table !!!
-- sdc_default                                 => détermine si les éléments doivent être sélectionnés par défaut (1 : oui / 0 : non)
-- sdc_visible                                  => spécifie si les éléments sont visibles à l'affichage (1 : oui / 0 : non)
-- sdc_master                                  => spécifie si les éléments sont présent uniquement dans le contexte du master ou il n'y a qu'un seul produit
-- sdc_truncate                               => spécifie si les éléments doivent supprimer avant d'insérer les nouveaux  (1 : oui / 0 : non) sert uniqment pour quand on monte un contexte
-- sdc_selected                               => spécifie si on peut sélectionner un (des) élément(s) ou alors on prend toutes la tables (1 : on peut sélectionner / 0 : on prend tous)
-- sdc_sql_where_select_default => condition pour la requête SQL qui récupère les éléments à afficher & condition pour la requête SQL pour savoir les éléments qui sont sélectionnés par défaut
--                                                           ATTENTION : la requête est exéctutée sur la première table !!!
...
--
-- Création de la table qui contiendra la liste des tables utilisés pour un élément de style graphe, dashboard...
--
-- sdctl_sdc_id       => identifiant d'un élément [sys_definition_context.sdc_id)
-- sdctl_sdct_id     => identifiant de la table utilisé pour un élément [sys_definition_context_table.sdct_id]
-- sdctl_select_all => spécifie si toutes les données de la table doivent être prises ou seulement les éléments concernés (1 : on prend tous / 0 : uniquement les éléments concernés),
--                                 Pour un élément ayant qu'une seule table alors la valeur n'a pas d'importance, ca dépend de la valeur de sys_definition_context.sdc_selected
...
--
-- Création du la table qui contiendra les éléments d'un contexte à supprimer
--
-- id => correspondra à l'identifiant unique à supprimer
...
--
-- Création d'une fonction pg/psql qui est appelé par des triggers placés sur des deletes
--
-- text : identifiant unique de l'élément qui doit être supprimé
...
--
-- Création d'une fonction pl/pgsql qui permet d'activer ou de désactiver un trigger sur une table
-- On utilise une fonction afin de ne pas générer d'erreur s'il n'y a pas de trigger sur la table
--
--
...
--
-- Modification des colonnes des identifiants pour qu'ils puissent acceptés du texte dans le cas des nouvelles façons de gérér les identifiants uniques ("internal_id")
--
...
-- Création d'une colonne id pour les niveaux d'agrégaton
...
-- on met à jour les menu deja existant
...
-- 25/03/2009 - MPR : BZ 8006 : Script de calcul d'alarmes itératives est trop long
...
-- Ajout d'une colonne dans la table sys_definition_flat_file_lib pour gérer les ordres de priorité dans la collecte des fichiers et identification des fichiers de référence
...
-- Ajout d'une colonne dans la table sys_definition_connection pour gérer l'affichage de la connexion sur le répertoire 'flat_file_zip'
	-- SYS_DEFINITION_CONNECTION
ALTER TABLE sys_definition_connection ADD protected int default 0;
-- Ajout d'une ligne dans la table sys_definition_connection pour gérer la connexion sur le répertoire 'flat_file_zip'
	-- SYS_DEFINITION_CONNECTION
...
-- 07/04/2009 BBX
-- Data Export doit être éxécuté également lors du compute Hour
...
--
-- Cette table permet par rapport un élément d'un contexte de définir les dépendances. (Exemple les dashboards doivent contenir les graphes)
-- 
-- sdcd_sdc_id                       => identifiant de l'élément parent
-- sdcd_sdc_id_dependency => identifiant de l'élément fils
-- sdcd_sdct_id                      => identifiant de la table qui sert de jointure
-- sdcd_column                       => nom de la colonne permettant de connaitre les éléments fils à sélectionner par rapport au parent
-- sdcd_column_join              => nom de la colonne permettant de faire la jointure sur la table avec la liste des éléments parents
-- sdcd_where                         => condition supplémentaires sur la table utilisé avec AND
--
...
-- 22/04/2009 GHX
-- Ajout de la BH dans le paramétrage contexte
...
-- SCT 14:24 15/05/2009 Mise à jour des séquences pour éviter les problèmes en fin de fichier SQL
SELECT setval('public.sys_definition_group_table_network_id_ligne_seq', (SELECT max(id_ligne)+1 from sys_definition_group_table_network), true);
SELECT setval('public.sys_definition_group_table_time_id_ligne_seq', (SELECT max(id_ligne)+1 from sys_definition_group_table_time), true);
SELECT setval('public.sys_definition_step_step_id_seq', (SELECT max(step_id)+1 from sys_definition_step), true);
...
-- 10/06/2009 MPR - Gestion des éléments par défault du context ( B9889 )
...
-- 16/06/2009 - MPR : Mise à jour de la colonne with_header ( valeur par défault à 1)
-- On passe à 1 la colonne with_header --
UPDATE sys_export_raw_kpi_config SET with_header = 1;

-- Ajout de la valeur par défault --
...
-- Modification dans le paramétrage contexte pour corriger le BZ 9848
...
-- 19/06/2009 BBX :
-- Modification des liens menu contextuel des alarmes pour gérer le produit
UPDATE menu_contextuel
...
-- Ajout de la table sys_definition_group_table_ref dans le paramétrage Parser du contexte
...
-- Modification de l'entrée sys_aa_interface pour le montage du contexte GPRS
...
-- Ajout la possibilité d'ajouter les Reports Schedules dans les contextes
-- 11:33 10/07/2009 GHX
-- Modif du paramétrage
...
INSERT INTO sys_debug (parameters, value, commentaire) VALUES ('retrieve_collect_data', 0, 'mode de debug de la collecte des fichiers pendant le retrieve [0 désactivé, 1 activé]');
DELETE FROM sys_debug WHERE parameters = 'retrieve_load_data';
INSERT INTO sys_debug (parameters, value, commentaire) VALUES ('retrieve_load_data', 0, 'mode de debug du chargement des fichiers pendant le retrieve [0 désactivé, 1 activé]');
DELETE FROM sys_debug WHERE parameters = 'retrieve_copy_from_temp_data';
INSERT INTO sys_debug (parameters, value, commentaire) VALUES ('retrieve_copy_from_temp_data', 0, 'mode de debug du chargement des tables temporaires pendant le retrieve [0 désactivé, 1 activé]');
...
-- 15:27 17/07/2009 
-- Ajout d'un mode débug pour le calcule des alarmes
DELETE FROM sys_debug WHERE parameters = 'alarm_calculation';
INSERT INTO sys_debug (parameters, value, commentaire) VALUES ('alarm_calculation', 0 , 'Mode de débug du calcule des alarmes (0 : désactivé / 1 : activé');
...
-- Ajout des colonnes "visible", "add_prefix" et "id_product" à la table sys_export_raw_kpi_config 
...
-- Midification des valeurs time period et offset par défaut. BZ 11094
-- sys_definition_master
TRUNCATE sys_definition_master;
INSERT INTO sys_definition_master (master_id, master_name, utps, offset_time, on_off, auto, visible, ordre, commentaire) VALUES (13, 'Compute Hourly', 30, 15, 0, false, 0, 5, 'ne pas parametrer laisser on_off a 0, lancé par id master 12');
INSERT INTO sys_definition_master (master_id, master_name, utps, offset_time, on_off, auto, visible, ordre, commentaire) VALUES (4, 'Compute', 1440, 780, 0, false, 0, 3, 'ne pas parametrer laisser on_off a 0, lancé par id master 11');
...
INSERT INTO sys_definition_master_ref (master_id, master_name, utps, offset_time, on_off, auto, visible, ordre, commentaire) VALUES (13, 'Compute Hourly', 30, 15, 0, false, 0, 5, 'ne pas parametrer laisser on_off a 0, lancé par id master 12');
INSERT INTO sys_definition_master_ref (master_id, master_name, utps, offset_time, on_off, auto, visible, ordre, commentaire) VALUES (4, 'Compute', 1440, 780, 0, false, 0, 3, 'ne pas parametrer laisser on_off a 0, lancé par id master 11');
...
-- On met certains éléments non visibles sur l'IHM
...
-- 18/09/2009 BBX : mise à jour des URLS de génération de doc des alarmes. BZ 11632
-- Export Word
UPDATE menu_contextuel 
