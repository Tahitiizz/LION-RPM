...
if [ "$type_install" != "installation" ]; then
	sysfieldreference restore
	sh $WORKING_DIR/scripts/restore_parameters.sh $dbname 
	sh $WORKING_DIR/scripts/restore_sys_definition_time_bh_formula.sh $dbname
	sh $WORKING_DIR/scripts/restore_sys_data_range_style.sh $dbname  
fi
