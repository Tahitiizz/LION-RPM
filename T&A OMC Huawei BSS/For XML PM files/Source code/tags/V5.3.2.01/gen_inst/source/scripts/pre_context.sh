...
# nettoyage des donnÃ©es contenues dans sys_flat_file_list_archive
chmod 777 $WORKING_DIR/scripts/clean_sys_flat_file_uploaded_list_archive.sh 
$WORKING_DIR/scripts/clean_sys_flat_file_uploaded_list_archive.sh
# fin de la partie nettoyage des donnÃ©es

# Sauvergarde de l'ancien contexte
...
for table in $( $psql_bin -U postgres $appname -c "select sdct_table from sys_definition_context_table;" -t) sys_versioning sys_definition_context_element_to_delete sys_definition_topology_replacement_rules; do
    cmd=$cmd" --table="$table" "
...
#rendre invisible tous les KPI Astellia : c’est le montage du contexte qui rendra visibles les KPI non obsolètes.
$SQL "UPDATE sys_definition_kpi SET visible=0 WHERE value_type='customisateur';" 2>&1
