...
<?
/*
	class alarmMailWithWord
	Permet d'envoyer les rapports word
*/
...
class alarmMailWithword{
	 ...
	 * @param $offset_day
	 */
	function alarmMailWithword ( $database_connection, $offset_day ) {
	...
	/**
	 * Enregistre le fichier word pour le user donnée dans sys_content_buffer
	 *
	 * @param $id_group
	 * @param $word_file_name
	 */
	function savewordforUser($id_group, $word_file_name){
	 ...
	 * @return string
	 */
	function wordBuilder($database_connection,$title,$file_name,$dir_saving_word_file,$na,$ta,$ta_value,$alarm_type,$sql_filter,$header,$isReport) {
		...
		for ($j=0;$j<count($critical_level);$j++) {
			...
			$word = new alarmCreateHTML($critical_level[$j],'condense','word',$queries,$title);
    ...
    /*08/06/2009 SPS ajout de quotes autour de l'id_user*/
		...
		if (pg_num_rows($result_search) > 0) {
			...
			$word_filepath	=  REP_PHYSIQUE_NIVEAU_0.get_sys_global_parameters("report_files_dir");
	...
	/**
	 * Construit le display et le word à envoyer (on les enregistre dans la table sys_content_buffer).
	 *
	 * @param  Array $tab_alarms
	 */
	function sendMailWithword($tab_alarms){
		...
		if($this->tabVide == false){
			...
			$dir_word = 				get_sys_global_parameters("pdf_save_dir");
			$dir_saving_word_file = 	$repertoire_physique_niveau0.$dir_word;
				...
				foreach($family_tab as $family => $na) {
					...
					foreach($na as $na_lib => $alarm_type_tab) {
						...
						foreach($alarm_type_tab as $alarm_type => $element) {
							...
							// On enregistre le word pour chaque groupe 'abonné'.
							...
							if($nombre_resultat_group_list > 0){
								...
								for ($i = 0;$i < $nombre_resultat_group_list;$i++){
									...
									// On génère le fichier word.

									if($this->flag_axe3){
										$file_name = $alarm_type."_".$family."_".$na_lib."_".$ta_lib."_".getTaValueToDisplayV2($ta_lib, $tab_values[$ta_lib],"_")."_".str_replace(" ", "_",$row["group_name"]).".word";
									}
									else {
										$file_name = $alarm_type."_".$family."_".$na_lib."_".$ta_lib."_".getTaValueToDisplayV2($ta_lib, $tab_values[$ta_lib],"_")."_".str_replace(" ", "_",$row["group_name"]).".word";
									}
									...
									$word_file_name = $this->Builder($this->database_connection,$title,$file_name,$dir_saving_word_file,'',$ta_lib,$tab_values[$ta_lib],$alarm_type,$sql_filter,$header,0);
									$word_file_name = $this->Builder($this->database_connection,$title,$file_name,$dir_saving_word_file,'',$ta_lib,$tab_values[$ta_lib],$alarm_type,$sql_filter,$header,0);
									...
									if(strlen(trim($word_file_name)) > 4){

										// On enregistre le fichier word pour le groupe dans sys_content_buffer.
										$this->savewordforUser($row["id_group"], $word_file_name);

										// On construit les emails pour chaque utilisateur du groupe courrant et on attache le fichier word que
										// l'on vient de créer.
										$query_list_user = "
										...
										if($nombre_resultat_list_user > 0){
											for ($j = 0;$j < $nombre_resultat_list_user;$j++){
												...
												// positionne le word comme pièce attaché dans le mail de l'utilisateur
												$word_attach = $dir_saving_word_file.$word_file_name;
												// Si la liste des alarmes du groupe précédent est égale à celle du groupe courrant alors, on n'a pas
												// besoins d'attacher le word.
												if($liste_id_alarmes_du_groupe_precedent != array($liste_id_alarmes_du_groupe,$user_mail)){
													if(!$mail_list[$user_mail]->AttachFileExist($word_attach)){
														$mail_list[$user_mail]->Attach($word_attach);
														echo "<ul><li>Fichier  : " . $word_attach . " attaché au mail <b>$user_mail</b></ul>";
														// On stock le nombre de résultats des alarmes courrantes.
														$nb_total[$user_mail] += $nb_resultat_des_alarmes;
													}
													else {
														echo "<ul><li><i>le fichier  : " . $word_attach . " a déjà été attaché au mail <b>$user_mail</b></i></ul>";
													}
												}
												else {
													echo "<ul><li>le word $word_file_name pour <b>$user_mail</b> n'a pas été attaché car un word avec les mêmes alarmes est déja attaché (mais pour un autre groupe).</ul>";
												}
											}
										}
									}
