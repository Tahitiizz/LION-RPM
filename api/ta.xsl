...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml" xmlns:ws="http://schemas.xmlsoap.org/wsdl/" xmlns:ws2="http://www.w3.org/ns/wsdl" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/wsdl/soap/" xmlns:local="http://tomi.vanek.sk/xml/wsdl-viewer" version="1.0" exclude-result-prefixes="ws ws2 xsd soap local">
    ...
    <xsl:param name="wsdl-viewer.version">1.0.0</xsl:param>
    ...
    <xsl:variable name="GENERATED-BY">Generated by Astellia</xsl:variable>
    ...
    */
    #header {
        ...
        height:76px;
        line-height:76px;
        background: #FFFFFF url(../images/bandeau2008/bg.png) repeat-y;
   ...
   <xsl:template match="ws:definitions | ws2:description" mode="html-title.render">
        <xsl:choose>
            <xsl:when test="$global.service-name">
                <xsl:value-of select="$global.service-name"/>
            </xsl:when>
            ...
            <xsl:otherwise>Web Service Fragment</xsl:otherwise>
             <!--
            <xsl:otherwise>
                <xsl:message terminate="yes">Syntax error in element <xsl:call-template name="src.syntax-error.path"/></xsl:message>
            </xsl:otherwise>
            -->
        </xsl:choose>
   </xsl:template>
   ...
   <xsl:template name="about.detail">
      ...
      <div>
        This page has been generated by <big>Astellia WSDL Viewer</big>, version <xsl:value-of select="$version"/>
      </div>
   </xsl:template>
   <xsl:template name="processor-info.render">
        <xsl:text></xsl:text>
        <xsl:text>This document was generated by Astellia</xsl:text>
   </xsl:template>
...
<xsl:template name="head.render">
      <head>
	        <title>
            <xsl:value-of select="concat($html-title, ' - ', 'Astellia WSDL Viewer')"/>
         </title>
...
<xsl:template name="footer.render">
      <div id="footer">
	This page was generated by Astellia (<a href="http://www.astellia.com">http://www.astellia.com</a>)
</div>
   </xsl:template>
