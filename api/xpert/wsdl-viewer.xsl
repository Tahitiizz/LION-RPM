...
-->
   ...
   <xsl:template name="render.source-code-link">
	     <xsl:if test="$ENABLE-SRC-CODE-PARAGRAPH and $ENABLE-LINK">
		       <!--<a class="local" href="http://tomi.vanek.sk/xml/%7Bconcat(%27#%27,%20$SRC-PREFIX,%20generate-id(.))}">
   ...
   <xsl:template name="processor-info.render">
      ...
      <a href="http://tomi.vanek.sk/xml/%7Bsystem-property(%27xsl:vendor-url%27)%7D">
...
<xsl:template match="ws:service|ws2:service" mode="service-start">
   ...
   <xsl:template match="ws:operation|ws2:operation" mode="service">
         ...
         <xsl:if test="$ENABLE-LINK">
		          <xsl:if test="$ENABLE-OPERATIONS-PARAGRAPH">
               <!--<a class="local" href="http://tomi.vanek.sk/xml/%7Bconcat(%27#%27,%20$OPERATIONS-PREFIX,%20generate-id(.))}">Detail</a>-->
            </xsl:if> 
            <xsl:call-template name="render.source-code-link"/>
         </xsl:if>
	     </li>
...
<xsl:template name="head.render">
      <head>
	        ...
	        <script src="wsdl-viewer.xsl_fichiers/wsdl-viewer.htm" type="text/javascript" language="javascript"> 
            <xsl:comment>
               <xsl:text>
	// </xsl:text>
            </xsl:comment>
