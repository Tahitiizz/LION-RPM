Ext.define('homepage.controller.charts.SummaryGraphEvo', {
	extend : 'Ext.app.Controller',
	views : ['charts.SummaryGraphEvo'],
	...
	load : function(config, time) {
		...
		var trend = Ext.getCmp(tabId + '_summary_graph_evo_trend');
		var grid = Ext.getCmp(tabId + '_summary_graph_evo_grid');
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				Ext.Ajax.request({
							url : 'proxy/alarm_list.php',
							params : {
								task : 'GET_WARNING_PENALTY_MONTH_EVO',
								params : {
									params : Ext.encode(alarmOptions)
								}
							},
							...
							success : function(response) {
								...
								Ext.Ajax.request({
									url : 'proxy/alarm_list.php',
									params : {
										task : 'GET_WARNING_INTERSECTION_MONTHS',
										params : {
											params : Ext.encode(alarmOptions)
										}
									},
									success : function(response) {
										var resultInter = Ext.decode(response.responseText);
										
										var trenddata_inter = new Array();
										var griddata_inter = new Array();
										//Push data into an array
										Ext.Object.each(resultInter, function(month, data) {
											trenddata_inter.push(data);
											data.reftime = Ext.Date.format(Ext.Date.add(Ext.Date.parse(month.toString()+ '01','Ymd'),Ext.Date.MONTH,(-1 * refPeriod)),'Ym');
											griddata_inter.push(data);
										});
