 ...
 * 
 * My Homepage / JGU2 :
 - la méthode getData doit être public pour le debug via Xdebug.
 - bug bloquant dans "getData" => getOperator devrait entourer de quote les ID qui ne sont pas des chiffres.
 - bug dans l'affectation de $colById[$na->id] : getData KO si l'élément réseau a un label différent de son ID => Clause where générée dans ce cas :
 AND ((CASE WHEN edw_object_ref.eor_label IS NOT NULL THEN edw_object_ref.eor_label ELSE edw_object_ref.eor_id END = 'EABBS01'))
 => la colonne utilisée dans le select est la même que celle utilisée dans le filter.
 => on n'obtient pas le label quand on fournit l'ID. 
 */
class QueryDataModel { 
	...
	private function api_getChild($query) {
		if (!$query) {
	...
	/** Retreive data and labels from the database
	 * @param $query object query receive from the API
	 * @return object response
	 */		
	private function api_getDataAndLabels($query) {
		if (!$query) {
			return $this->getJsonError(7, 'No parameter found.', null);			
		}				
		return $this->getDataAndLabels($query);
	}
	...
	private function api_getData($query) {
		...
		return $this->getData($query,$roaming);
	}
	...
	public function getDataAndLabels($query) {
		// produit sur le quel porte la requete
		$targetedProductId = '';
		
		// on recherche les labels des compteurs et KPI
		$labels = '';
		
		$selectedElements = $query->select;
		
		/**
		var_dump('DEBUT');
		var_dump($selectedElements);
		var_dump('FIN');
		**/
		foreach ($selectedElements->data as $index => $element) {
			switch($element->type) {
				case 'RAW':
					$rawModel = new RawModel();
					$rawLabel = $rawModel->getLabelFromId($element->id, Database::getConnection($element->productId));
					if(!isset($rawLabel) || $rawLabel==FALSE) {
						$rawLabel = $element->id;
					}
					$labels .= '{"type":"RAW","id":"'.$element->id.'","productId":"'.$element->productId.'", "label":"'.$rawLabel.'"},';
					if ($targetedProductId == '') $targetedProductId = $element->productId;
					break;
					
				case 'KPI':					
					$kpiModel = new KpiModel();
					$kpiLabel = $kpiModel->getLabelFromId($element->id, Database::getConnection($element->productId));
					// si kpi non trouve
					if(!isset($kpiLabel) || $kpiLabel==FALSE) {
						$kpiLabel = $element->id;
					}
					$labels .= '{"type":"KPI","id":"'.$element->id.'","productId":"'.$element->productId.'", "label":"'.$kpiLabel.'"},';
					if ($targetedProductId == '') $targetedProductId = $element->productId;
					break;
		...
		/**
		var_dump('DEBUT');
		var_dump($query->filters);
		var_dump('FIN');
		**/
		// on recherche les labels des eventuels elements reseaux recherches
		foreach ($query->filters->data as $index => $element) {
			if($element->type == 'na') {
				// ex : {"type":"na","id":"bsc","value":"EABBS01"}
				$neId = $element->value;
				$networkLevelCode = $element->id;
				$neLabel = NeModel::getLabel($neId, $networkLevelCode, $targetedProductId);
				if(!isset($neLabel) || $neLabel==FALSE) {
					$neLabel = $neId;
				}
				// ex : {"type":"ne","id":"EABBS01","level":"bsc","label":"My EABBS01"}
				$labels .= '{"type":"ne","id":"'.$neId.'","na":"'.$networkLevelCode.'","productId":"'.$targetedProductId.'", "label":"'.$neLabel.'"},';
			}
			else if($element->type == 'na_axe3') {
				// ex : {"type":"na","id":"bsc","value":"EABBS01"}
				$neId = $element->value;
				$networkLevelCode = $element->id;
				$neLabel = NeModel::getLabel($neId, $networkLevelCode, $targetedProductId);
				...
				// ex : {"type":"ne","id":"EABBS01","level":"bsc","label":"My EABBS01"}
				$labels .= '{"type":"ne","id":"'.$neId.'","na_axe3":"'.$networkLevelCode.'","productId":"'.$targetedProductId.'", "label":"'.$neLabel.'"},';
			}
		...
		// valeurs des compteurs/KPI
		$data = $this->getData($query);
		
		// Construction de la chaine json à partir de l'objet data
		...
		// resultat
		$jsonResult = '{"labels":['.$labels.'],"values":'.$datas.'}';
		return $jsonResult;
	}
	...
	public function getChild($query) {
		$father_id = $query->father->id;
		$product_id = $query->father->productId;
		$sql = "select source_default from sys_definition_network_agregation where agregation_name = '".$father_id."';";
		$database = DataBase::getConnection($product_id, true);
		$child_id =null;
		if($database->getCnx()) {
				$i = 0;
	 ...
	 */	
	public function createSqlQuery($query, $graphTaAxis = null, $roaming = false) {								
		$orderBy = array();
		try {
			...
			// If this is a single product query
				...
				foreach($productIds as $productId) {
					...
					$bean = $this->createSqlQueryForSingleProd($query, $graphTaAxis, $isMultiProdQuery, $productId, $roaming);
	 ...
	 */	
	private function getNa3Value($na3List, $na, $firstTableName, $currentProductId, $roaming,$mapid) {
		$id = $na->id;
...
//				$value = '(SELECT eor_label FROM edw_object_ref WHERE eor_id = '.$firstTableName.'.'.$id.' LIMIT 1)';
				...
				//$value = 'COALESCE((SELECT eor_id FROM edw_object_ref WHERE eor_id='.$firstTableName.'.'.$id.' LIMIT 1),'.$firstTableName.'.'.$id.')';
				
				if($roaming == false){
					$value = 'COALESCE((SELECT eor_label FROM edw_object_ref WHERE eor_id='.$firstTableName.'.'.$id.' LIMIT 1),'.$firstTableName.'.'.$id.')';
				}else{
					$value = 'COALESCE((SELECT eor_id FROM edw_object_ref WHERE eor_id='.$firstTableName.'.'.$id.' LIMIT 1),'.$firstTableName.'.'.$id.')';
				}
			...
			if ($na->valueType == 'code') {
				...
				if($roaming == false){
					if($mapid == false){
						...
						$value = "COALESCE((SELECT get_na_parent_id($firstTableName.$minLevel, '$minLevel', '$id', '$fam')))";
					}	
				}else{
					$value = "COALESCE((SELECT get_na_parent_id($firstTableName.$minLevel, '$minLevel', '$id', '$fam')))";
				}
	 ...
	 * @return SQL string to get the NA3 value
	 */		
	private function getNaValue($naMin, $na, $bean,$mapid,$roaming=false) {		
		$id = $na->id;
		// For filter with 'In' or 'Not in' operator the filter is apply on 'code'
		if (isset($na->isFilter) && ($na->operator == 'In' || $na->operator == 'Not in')) {
		 	$na->valueType = 'code';
		...
		if ($naMin == $id ) {
			if ($na->valueType == 'label') {	// Display label
				$value = 'edw_object_ref.eor_label';
			}
			// The value is available in the edw_object_ref
			elseif ($na->valueType == 'code' || $mapid == true) {			// Display code
		...
		// This is not the NA min
			...
			if ($na->valueType == 'code' || $mapid == true) {
	...
	private function computeNaElements($bean, $query, &$colById, &$columnHeader, &$idLabel, &$select_list) {
		...
		if(isset($query->roaming)){
			$roaming = $query->roaming;	
		}else{
			$roaming = false;
		}
		...
		foreach ($bean->getNaElements() as $na) {
			...
			if ($na->type == 'na_axe3') {					
				// Get NA3 value
				$value = $this->getNa3Value($na3, $na, $firstTableName, $bean->getCurrentProductId(),$roaming,$mapid);
				$isNaMin = false;
			} else {
				// Get the NA value
				$isNaMin = ($id == $bean->getNaMin());
				$value = $this->getNaValue($bean->getNaMin(), $na, $bean,$mapid,$roaming);					 					
			}
			
			// Save the column value for filters			
			$colById[$na->id] = $firstTableName.'.'.$id;
			// $colById[$na->id] = $value; ?
			...
			if (!isset($na->isFilter)) {			
				...
				$na->sqlColumnName = $na->id.$idLabel;
	...
	private function computeTaElements($bean, $query, $formatbh = false, &$colById, &$columnHeader, &$idLabel, &$select_list) {
		$orderBy = $bean->getOrderBy();
		$groupBy = $bean->getGroupBy();
		...
		foreach ($bean->getTaElements() as $element) {						
			...
			if($formatbh == false){
				if($element->id == 'day_bh' || $element->id == 'week_bh' || $element->id == 'month_bh'){
					$value = '{mainTable}.'.'bh';			// maintable will be compute later
				}else{
					$value = '{mainTable}.'.$element->id;			// maintable will be compute later
				}
			}else{
			...
			if (!isset($element->isFilter)) {											
				...
				// Add a unique name for this column	
				//$element->sqlColumnName = $element->id.$idLabel;
				if($formatbh == false){
					if($element->id == 'day_bh' || $element->id == 'week_bh' || $element->id == 'month_bh'){
						$element->sqlColumnName = 'bh'.$idLabel;
					}else{
						$element->sqlColumnName = $element->id.$idLabel;
					}
				}else{
	...
	private function computeRawKpi($bean, $query, &$columnHeader, &$idLabel, &$select_list, &$fromTable, &$where, &$mainTable) {		
		...
		foreach($bean->getRawkpis() as $element) {
			...
			if ($bean->getIsMultiProduct() && $element->productId != $bean->getCurrentProductId()) {
				...
				// Add a unique name for this column
				$element->sqlColumnName = $element->name.$idLabel;
				$value= 'CAST(null AS real) AS '.$element->name.$idLabel++;					
		...
		if (isset($query->filters) && isset($query->filters->data)) {
			foreach($query->filters->data as $element) {									
				...
				if (!isset($element->enable) || $element->enable) {
					...
					}else if ($element->type == 'sys' && isset($element->date)){
						$operators[] = $this->getFilter($name, $element);
						$operators[] = ' '.((isset($element->connector) && $element->connector)?"\n\t".$element->connector:"\n\t AND").' ';
					}
					else {
	 ...
	 */	
	public function createSqlQueryForSingleProd($query, $graphTaAxis = null, $isMultiProduct, $currentProductId, $roaming = false) {				
		if(isset($query->formatbh)){
			$formatbh = $query->formatbh;	
		}else{
			$formatbh = false;
		}
		...
		try {
			...
			$this->computeTaElements($bean, $query,$formatbh, &$colById, &$columnHeader, &$idLabel, &$select_list);														
