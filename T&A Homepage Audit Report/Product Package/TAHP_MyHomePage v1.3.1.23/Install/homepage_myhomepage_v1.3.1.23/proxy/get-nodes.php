...
$path = isset($_REQUEST['path'])&&$_REQUEST['path'] == 'archives' ? '../../' : '../';
//var_dump($path);
$node = isset($_REQUEST['node']) ? $_REQUEST['node'] : 'archives';
$isXml = isset($_REQUEST['isXml']);

if(strpos($node, '..') !== false){
    die('You can\'t go further');
}
...
if (is_dir($directory)){
    ...
    while($f = $d->read()){
        ...
        $lastmod = date('M j, Y, g:i:s a', filemtime($filename));
		$curdate = date('M j, Y, g:i:s a');
...
if ($isXml) {
    ...
    foreach ($nodes as $node) {
		...
		$xmlNode->appendChild($xmlDoc->createElement("date", $node['date']));
