...
Ext.define('Ext.ux.chart.LegendItem', {
    ...
    createLegend: function(config) {
        ...
        me.clickable=config.legend.clickable;
        me.showline=config.legend.showline;
        
        
        if(me.clickable==true){
        	// Add event listeners if clickable
            me.on('mouseover', me.onMouseOverExtended, me);
            me.on('mouseout',  me.onMouseOutExtended,  me);
            me.on('mousedown', me.onMouseDownExtended, me);
        }
        else{
        	// remove event listeners if not clickable
        	me.un('mouseover', me.onMouseOverExtended, me);
            me.un('mouseout',  me.onMouseOutExtended,  me);
            me.un('mousedown', me.onMouseDownExtended, me);
        }
    ...
    createLabel: function(config) {
        ...
        return me.add('label', me.surface.add({
            ...
            fill: 
            (config.legend.style == 'access' ? '#FFFFFF' : legend.labelColor),
    ...
    createMask: function(config) {
        ...
        return me.add('mask', surface.add({
            ...
            y: bbox.y,
            width: bbox.width || 20,
            height: /**bbox.height || 20**/0,
            zIndex: (me.zIndex || 0) + 1,
            fill : 
            	(config.legend.style == 'access' ? 'none' : legend.boxFill),
            style: 
            	(me.clickable ? {cursor: 'pointer'} : {})
        }));
    },
    ...
    onMouseOverExtended: function() {
        ...
        if (!me.showline) {
        	...
        	curentNe = me.series.title;
        	for (i = 2, items = me.legend.chart.series.items, l = items.length; i < l; i++) {
        		if(items[i].title == curentNe){
        			me.legend.chart.series.items[i].unHighlightItem(items[i]);
        		}else{
        			me.legend.chart.series.items[i].highlightItem(items[i]);
        		}
        	}
        }
    },
