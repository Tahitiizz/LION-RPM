	...
	08/04/2009 - modif SPS : on teste si ie6 ou inferieur pour appliquer des modifs sur le style (image tab_right* mal placee)

	************************************************************************************************************/
	var textPadding = 3; // Padding at the left of tab text - bigger value gives you wider tabs
	var strictDocType = true;
	var tabView_maxNumberOfTabs = 6;	// Maximum number of tabs
	var imgUrl; // chemin vers le dossier contenant les images
	...
	function showTab(parentId,tabIndex)
	{
		...
		if(activeTabIndex[parentId]>=0){
			...
			img.src = imgUrl+'tab_right_inactive.gif';
		...
		img.src = imgUrl+'tab_right_active.gif';
	...
	function rolloverTab()
	{
		if(this.className.indexOf('tabInactive')>=0){
			...
			img.src = imgUrl+'tab_right_over.gif';
		}
	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,additionalTab,linkToImg)
	{
        ...
        {
            // Si le nom de l'onglet est vide, on ne le gère pas (robustesse)
            if( tabTitles[no].length > 0 )
            {
                ...
                // 13/08/2010 NSE DE Firefox bz 16905 : on repositionne l'image de droite et le texte des onglets
                ...
                // IE5.X FIX
                /* 08/04/2009 - modif SPS : on teste si ie6 ou inferieur pour appliquer des modifs sur le style*/
                if((navigatorVersion && navigatorVersion<7) || (MSIE && !strictDocType)){
	...
	function createNewTab(parentId,tabTitle,tabContent,tabContentUrl)
	{
		...
		var tabId = initTabs(parentId,Array(tabTitle),0,'','',true,imgUrl);
