...
database=$2
base_exists=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin template1 -c "\l" | grep $database | wc -l)
if [ "$base_exists" == "0" ]
then
	echo '###################################################################################################'
...
SQL="env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $database -tAc "
...
if [ "$type_installation" == "update" ] ; then
	...
	if [ "$(ls *.sql 2>/dev/null| wc -l)" != "0" ]; then
		...
		do
			...
			cat $sql_file | env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin $database -L $LOG 2>&1
	...
	if [ "$(ls *.sql 2>/dev/null| wc -l)" != "0" ]; then
		...
		do
			echo -e "\t+ Updating database, executing '$sql_file'"
			cat $sql_file | env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin $database >> $LOG 2>&1
		done
	fi
fi
