...
exit_if_process_running() {
	...
	process=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT master_name FROM sys_process_encours, sys_definition_master WHERE encours = 1 AND process::text = master_id::text LIMIT 1" 2>>$LOG)
	if [ -n "$process" ]
	then
		cat <<EOF
...
exit_if_active_counters_not_deployed() {
	[ -d "/home/$appname" ] || return # ce n'est pas un cas de migration ? alors rien a verifier
	nb_counter=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT count(edw_field_name) FROM sys_field_reference WHERE on_off = 1 AND new_field = 1" 2>>$LOG)
	if [ "$nb_counter" -gt 0 ]
	then
		cat <<EOF
$nb_counter counter(s) are currently activated but not deployed for $appname.
Please retrieve some date to allow it to be deployed, and relaunch this script.
EOF
        add_log "$nb_counter counter(s) are currently activated but not deployed for $appname. Please retrieve some date to allow it to be deployed, and relaunch this script."
		clean_exit
	fi
}
...
sysdefmaster() {
	...
	if [ "$1" = backup ] ; then
		# env PGPASSWORD=$postgresqlPassword /usr/local/pgsql/bin/psql -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "select master_id from sys_definition_master where on_off = 1 and visible = 1" > "$sdf_bkp_file" 2>>$LOG
		env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "select master_id, utps, offset_time, on_off from sys_definition_master where on_off = 1 and visible = 1" > "$sdm_bkp_file" 2>>$LOG
		env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "select master_id, utps, offset_time, on_off from sys_definition_master_ref where on_off = 1 and visible = 1" > "$sdmr_bkp_file" 2>>$LOG
		env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -atAc "update sys_definition_master set on_off = 0 where on_off = 1 and visible = 1" >> $LOG 2>&1
	elif [ "$1" = restore ] ; then
		if test -r "$sdm_bkp_file" -a -s "$sdm_bkp_file"; then
			cat "$sdm_bkp_file" | awk -F '|' '{printf("UPDATE sys_definition_master SET utps = %s, offset_time = %s, on_off = %s WHERE master_id = \047%s\047;\n", $2, $3, $4, $1);}' | env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname >> $LOG 2>&1
		fi
		...
		#env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -atAc "update sys_definition_master set on_off = 1 where master_id in ($list)" >> $LOG 2>&1
		rm -f "$sdm_bkp_file"
		rm -f "$sdmr_bkp_file"
	else
...
sysfieldreference() {
	if [ "$1" = backup ] ; then
		env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT edw_field_name, edw_group_table , on_off FROM sys_field_reference" > "$sfr_bkp_file" 2>>$LOG
	elif [ "$1" = restore ] ; then
		test -r "$sfr_bkp_file" || return
		test -s "$sfr_bkp_file" || return
		chmod 777 "$sfr_bkp_file"
		#env PGPASSWORD=$postgresqlPassword /usr/local/pgsql/bin/psql -U $postgresqlLogin -d $dbname -p $postgresqlPort -atAc "COPY sys_field_reference (id_ligne,on_off) FROM '$sfr_bkp_file' WITH DELIMITER '|'" >> $LOG 2>&1
		cat "$sfr_bkp_file" | awk -F '|' '{if($3==1)printf("UPDATE sys_field_reference SET on_off = %s WHERE edw_group_table = \047%s\047 AND edw_field_name = \047%s\047;\n", $3, $2, $1);}'| env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname >> $LOG 2>&1
		rm -f "$sfr_bkp_file"
	else
...
set_rights(){ 
   # Changement des droits sur les répertoires et les fichiers
   find "/home/$appname" -type d -exec chmod 770 {} \;
   find "/home/$appname" -type f -exec chmod 660 {} \;
   # Affectation du repertoire au user apache du groupe apache
   chown -R astellia:astellia "/home/$appname"   
   add_log "Modification des droits sur les répertoires et fichier de l'application"
}

# Affichage de messages d'attente
please_wait()
{
  current_action=""
  [ ! -z "$1" ] && current_action=$1

  current_width=40
  [ ! -z "$2" ] && current_width=$2
  msg_title="Info"
  test_dialog=$(which "$DIALOG" > /dev/null 2>&1 ; echo $?)
  if [ "$test_dialog" == "0" ] ; then
    $DIALOG --backtitle "$msg_title" --infobox "Please wait...\n$current_action" 5 "$current_width"
  else
    echo -e "Please wait...\n$current_action"
  fi
}
