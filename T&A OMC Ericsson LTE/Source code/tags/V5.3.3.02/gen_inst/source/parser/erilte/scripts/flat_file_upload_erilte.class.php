...
include_once(REP_PHYSIQUE_NIVEAU_0 . "parser/$module/scripts/DatabaseServicesEriLte.class.php");
...
class parser_upload extends Collect
{
	...
	function __construct()
	{
		...
		$dBServices=new DatabaseServicesEriLte($dbConnection);
	...
	public function getFiletime(FlatFile $flatFile){
		...
		$pattern="/A([0-9]{4})([0-9]{2})([0-9]{2})\\.([0-9]{2})([0-9]{2})([+,-][0-9]{2})[0-9]{2}\\-([0-9]{2})([0-9]{2})([+,-][0-9]{2})[0-9]{2}_SubNetwork=(.*),SubNetwork=(.*),MeContext=(.*)_statsfile.xml/i";
		...
		}elseif( preg_match($pattern1, $fileName, $regs)){
			$s_year=$regs[1];
			$s_month=$regs[2];
			$s_day=$regs[3];
			$s_hour=$regs[4];
