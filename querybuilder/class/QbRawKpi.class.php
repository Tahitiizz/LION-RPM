	...
	private function getElements($type, $searchOptions) {		
		...
		if ($searchOptions) {
			if ($searchOptions->text) {
				...
				// SPD1 23/02/2012 : Escape special char BZ 25876		
				$searchOptions->text = addcslashes($searchOptions->text, '%\\_');
				$searchOptions->text = pg_escape_string($searchOptions->text);
		...
		foreach ($allProducts as $prod) {
			...
			if ($searchOptions) {
				...
				if (!isset($productList[$prod['sdp_id']])) {
					continue;
				}							
			...
			if (isset($productFilterOptions->families)) {
				...
				$productFilter = " and sdc.family in ($families)";
			}						
