...
<?php
/*
This script is supposed to be used together with HTML2FPDF.php class
...
//(check XHTML2PDF code for more size types?)

  //Identify size (remember: we are using ´mm´ units here)
  if ( strstr($size,'px') ) $size = (($size*25.4)/72);//conversion pixel -> mm in 72dpi
  elseif ( strstr($size,'cm') ) $size *= 10;
  elseif ( strstr($size,'mm') ) $size += 0;
  elseif ( strstr($size,'pt') ) $size += 0;//??
  elseif ( strstr($size,'%') )
  {
  ...
  else $size = $size; //if none of the above consider being what?//dividir por 4?
