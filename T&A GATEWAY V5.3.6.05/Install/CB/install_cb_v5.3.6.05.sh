...
is_php5=`${php_dir}php -v | head -n1 | grep -E "^PHP 5.2.|^PHP 5.3." | wc -l`
if [ $is_php5 != "1" ] ; then
...
if [ $user = 'root' ]; then
	...
	base_exists=`env PGPASSWORD=$postgresqlPassword $''$psql_dir$''/psql -U $postgresqlLogin template1 -c "\l" | grep $'\ '$database$'\ ' | wc -l`
	...
	# LAUNCHING INSTALLATION ####################################################################################
       ...
       #current_version=`env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -c "SELECT item_value FROM sys_versioning WHERE item = 'cb_version' ORDER BY date DESC LIMIT 1" | head -n3 | tail -n1 | sed 's/ //g' | grep -E "[0-9].[0-9]..*"`
	...
	sed -i -r "s/(\\\$AUser\s*=\s*\")(.*)(\")/\1$postgresqlLogin\3/g" xbdd.inc
	sed -i -r "s/(\\\$APass\s*=\s*\")(.*)(\")/\1$postgresqlPassword\3/g" xbdd.inc
	...
	#############################################
	# ADD postgres and apache to astellia group #
	#############################################
  # Ajout de apache au groupe astellia
  usermod -aG astellia apache
  # Ajout de postgres au groupe astellia
  usermod -aG astellia postgres 
  ...
  ###############
	...
	if [ $base_exists != "1" ] ; then
		...
		env PGPASSWORD=$postgresqlPassword $psql_dir/psql -tc "select procpid from pg_stat_activity where datname='template1'" -U $postgresqlLogin -d postgres | xargs -r kill && env PGPASSWORD=$postgresqlPassword $psql_dir/createdb -U $postgresqlLogin -E 'SQL_ASCII' $database -T template1 >> $rep_install'/SQL/'$file_install_log 2>&1

		# IF CREATION FAILED, THROWING ERROR THEN EXIT
		db_is_created=`env PGPASSWORD=$postgresqlPassword $''$psql_dir$''/psql -U $postgresqlLogin template1 -c "\l" | grep $'\ '$database$'\ ' | wc -l`
		...
		zcat $cb_bdd | tail -n+2 | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -q  -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1

		# UPGRADING PRODUCT INFORMATION
		echo "TRUNCATE TABLE sys_definition_product" | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
		...
		echo "INSERT INTO sys_definition_product (sdp_id,sdp_label,sdp_ip_address,sdp_directory,sdp_db_name,sdp_db_port,sdp_db_login,sdp_db_password,sdp_on_off,sdp_master,sdp_master_topo,sdp_ssh_user,sdp_ssh_password,sdp_ssh_port) VALUES (1,'$alias','$server_ip_address','$rep_install_sans_home','$database','$postgresPort','$postgresqlLogin','$postgresqlPassword',1,1,1,'astellia','astellia',22)" | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
		echo '##################################################' >> $rep_install'/SQL/'$file_install_log 2>&1
		...
		echo '*/10 12-23 * * 0 '$php_dir'php -q '$rep_install'/scripts/save_database.php &> /dev/null' >> $chemin_crontab
	...
	#####################
		...
		if [ $is_migration_40 == "1" ] ; then
			...
			env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -f $rep_install/SQL/migrationGTMDashboardCB41.sql >> $rep_install'/SQL/'$file_install_log 2>&1
			env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -f $rep_install/SQL/message_display.sql >> $rep_install'/SQL/'$file_install_log 2>&1
			env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -f $rep_install/SQL/cb_v5.0.0.00.sql >> $rep_install'/SQL/'$file_install_log 2>&1
			...
			do
				...
				case $EXTENSION in
					...
					sql)
						...
						echo '##################### SQL ########################' >> $rep_install'/SQL/'$file_install_log 2>&1
						cat $script | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -q  -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
						echo '##################################################' >> $rep_install'/SQL/'$file_install_log 2>&1
	...
	echo '##################### SQL ########################' >> $rep_install'/SQL/'$file_install_log 2>&1
	version_exists=`env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -c "SELECT id FROM sys_versioning WHERE item = 'cb_version' AND item_value = '$version_cb' AND item_mode = 'version_de_base'" | head -n3 | tail -n1 | grep "|" | wc -l`
	if [ $version_exists == "0" ] ; then
		echo "INSERT INTO sys_versioning (item,item_value,item_mode,date) values ('cb_name','$name_cb','version_de_base','$date')" | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
		echo "INSERT INTO sys_versioning (item,item_value,item_mode,date) values ('cb_version','$version_cb','version_de_base','$date')" | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
	fi
	echo '##################################################' >> $rep_install'/SQL/'$file_install_log 2>&1
	echo '-> Done.' >> $rep_install'/SQL/'$file_install_log 2>&1
	echo '--------------------------------------------------' >> $rep_install'/SQL/'$file_install_log 2>&1
	...
	# UPDATE SYS_DEBUG
	echo "UPDATE sys_debug SET value=0" | env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database >> $rep_install'/SQL/'$file_install_log 2>&1
	...
	if [ $base_exists == "1" ] ; then
		...
		is_master=`env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -c "SELECT sdp_directory, sdp_db_name FROM sys_definition_product WHERE sdp_db_name = '$database' AND sdp_master = 1 " | head -n3 | tail -n1 | grep "|" | wc -l`
		is_mk=`env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -c "SELECT sdp_directory, sdp_db_name FROM sys_definition_product WHERE sdp_db_name LIKE 'mixed_kpi%'" | head -n3 | tail -n1 | grep "|" | wc -l`
		# IF MIXED KPI PRODUCT EXISTS
		if [[ $is_mk == "1" && $is_master == "1" ]] ; then
			# FETCHING DIR AND DB NAME
			mk_infos=`env PGPASSWORD=$postgresqlPassword $psql_dir/psql -U $postgresqlLogin $database -c "SELECT sdp_directory, sdp_db_name FROM sys_definition_product WHERE sdp_db_name LIKE 'mixed_kpi%'" | head -n3 | tail -n1 | grep "|" | sed "s/ //g"`
