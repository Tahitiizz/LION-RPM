...
archive="ta_gateway_5.3.6.05.tar.gz"
parser_name="bp"
default_appname="gateway"
product_name="gateway"
product_label="TA Gateway"
product_version="5.3.6.05"
pkgmd5="b66bcdb555592cc5d07d988b275237bc"
type_install="installation"
install_order="cb_base;parser;context"
gen_inst_version="3.16" # 11/02/2011 11:05 SCT : ajout de la version du gen_inst utilisé pour faire le package produit dans le fichier de log
min_previous_product_version="" # version minimale requise pour installation, optionnel
is_ok=true # 05/04/2017 : variable permettant de savoir si on peut continuer ou non l'installation
...
PSQL="env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -tAc "
WORKING_DIR=$PWD
...
PSQL="env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -tAc "
...
#
# ==== POSTGRESQL ACCOUNT ====
add_log "Postgresql account verification" "title"
check_postgresql_connection
if [ "$connectionState" -eq 0 ] ; then 
	# It isn't possible to log in using the new PostgreSQL account
	add_log "PostgreSQL default account is still being used."
	add_log "Creating PostgreSQL account '$postgresqlLogin'."
	...
	check_postgresql_connection
	if [ "$connectionState" -eq 1 ] ; then
		add_log "Connection to database with the user $postgresqlLogin succeed."
	else
		MSG="Connection to database with the user $postgresqlLogin failed. Please try again."
		show_msg "$MSG" "Error"
		add_log "$MSG"
		clean_exit
	fi
fi	
if grep -v '^\s*#' /usr/local/pgsql/data/pgdata/pg_hba.conf | grep -q 'all\s\+0\.0\.0\.0\/0\s\+trust' ; then
	# PostgreSQL is configured to allow anyone to connect from anywhere without password 
	# Change the client authentication settings of PostgreSQL
	add_log "PostgreSQL configuration isn't secured."
	add_log "Modifying PostgreSQL configuration."
	cp -f $WORKING_DIR/quick_install_tools/pg_hba.conf $pg_data_dir/pg_hba.conf >> $LOG 2>&1
	chown postgres.postgres $pg_data_dir/pg_hba.conf >> $LOG 2>&1
	# Load the new client authentication settings of PostgreSQL
	tmp=$($PSQL "SELECT pg_reload_conf();" -d postgres)
fi
add_log "...done"
...
then
    ...
    do
        ...
        if [ -f "${listDirectory[$compteur]}php/xbdd.inc" ] ; then
            ...
            tableExists=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $datname -p $postgresqlPort -tAc "SELECT COUNT(*) FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('sys_global_parameters');")
            if [ "$tableExists" = 1 ]; then
                valeurModule=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $datname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='module';")
                productVersion=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $datname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='product_version';")
                ...
                if [ "$valeurModule" = "$parser_name" ]; then
                    ...
                    valeurOldModule=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $datname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='old_module';")
...
if [ "$type_install" != "installation" ] && [ "$choiceType" = "manual appname" ] ; then
    valeurModuleOnSelectApp=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='module';")
    if [ "$valeurModuleOnSelectApp" = "def" ] ; then
        valeurModuleOnSelectApp=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='old_module';")
    fi
...
then
	patchedAppVersion=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='product_version';")
...
base_exists=$($PSQL "SELECT count(*) FROM pg_database WHERE datname='$dbname';" -d postgres)
MSG_CHECK=''
...
#==============================Test version CB==================================
#===============================================================================
if [ "$product_name" != "gateway" ] ; then
   postgresqlPassword=$(echo $postgresqlEncryptedPassword | openssl aes-256-ecb -d -a -salt -k "$postgresPasskey")
   if gateway_info=$(PGPASSWORD=$postgresqlPassword psql -U $postgresqlLogin -p $postgresqlDefaultPort -d $dbname -h $server_ip_address -tAc "SELECT sdp_db_name, sdp_ip_address FROM sys_definition_product WHERE sdp_directory LIKE '%gateway%' OR sdp_label LIKE '%Gateway%';" ); then
       IFS='|' read -a arr_gateway_info <<< "${gateway_info}"
       # Récupération du nom de la base            
       sdp_db_name=${arr_gateway_info[0]}
       sdp_ip_address=${arr_gateway_info[1]}        
       if [ "$sdp_db_name" != "" ] && [ "$sdp_ip_address" != "" ] ; then
          # On récupère la dernière version du CB dans la table sys_versionning de la gateway
          if last_gateway_cb_version=$(PGPASSWORD=$postgresqlPassword psql -U $postgresqlLogin -p $postgresqlDefaultPort -d $sdp_db_name -h $sdp_ip_address -tAc "SELECT item_value FROM sys_versioning WHERE item = 'cb_version' ORDER BY id DESC LIMIT 1;" ); then
              # On compare la version du CB de la gateway avec celle du produit
              arr_last_gateway_cb_version_value=(${last_gateway_cb_version//./ })
              
              # Récupération de la version CB du produit
              arr_product_cb_version=(${cb_final_ver//./ })
              
              for index in "${!arr_last_gateway_cb_version_value[@]}"; 
                  do                     
                     if [ ${arr_product_cb_version[index]} -gt ${arr_last_gateway_cb_version_value[index]} ]; then 
                        # Versions CB incompatibles
                        is_ok=false
                     fi
                  done
          fi
   ...
   # La version CB du produit à installer est supérieure à celle de la version du CB de la Gateway
   if [ "$is_ok" != true ]; then
      MSG="The version of this CB $cb_final_ver is upper than the Gateway version $last_gateway_cb_version. \nPlease upgrade Gateway version before install this product. \nAborting installation."
      show_msg "$MSG" "Error"
      add_log "The version of this CB $cb_final_ver is upper than the Gateway version $last_gateway_cb_version."
      clean_exit           
  fi
fi
...
SQL="env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -L $LOG -d $dbname -tAc "
SQLFILE="env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -L $LOG -d $dbname -f "
...
if [ "$type_install" = "migration" ] || [ "$type_install" = "patch" ]; then
    ...
    poubelle=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -L $LOG -d $dbname -c "SELECT * FROM sys_versioning;" 2>&1) >> $LOG
fi
...
export SQL WORKING_DIR type_install appname dbname LOG DIALOG QUIET psql_bin postgresqlPort postgresqlLogin postgresqlPassword
...
if [ "$type_install" = "installation" ]; then
	...
	nb_cx=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -tAc "SELECT count(connection_name) FROM sys_definition_connection WHERE connection_directory='/home/$appname/flat_file'" $dbname |head -1|sed 's/\s*//')
...
if [ "$type_install" != "patch" ]; then
    ...
    topo=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -tAc "SELECT value FROM sys_global_parameters WHERE parameters='topology_file_location';" $dbname|sed 's/ //'|head -1)
...
plabel=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -p $postgresqlPort -tAc "SELECT sdp_label FROM sys_definition_product WHERE sdp_directory='$appname';" $dbname|sed 's/ //'|head -1)
	...
	if [ ! -f "$paaConfFile" ]; then
		...
		if [ $QUIET = "false" ]; then
            ...
            if [ ! -z "$sqlite_session_file" ]; then
                ...
                add_log "Updating Modification des droits sur $sqlite_dir et $sqlite_session_file"
            else
		...
		case $res in
			0)
                ...
                if [ "$allowUserCustomizeHomepage" = 1 ] ; then
                    ...
                    listNa=$(env PGPASSWORD=$postgresqlPassword $psql_bin -U $postgresqlLogin -d $dbname -p $postgresqlPort -tAc "SELECT agregation FROM sys_definition_network_agregation WHERE family = '$homepageFamily' ORDER BY agregation_level;")
                    for oneNa in $listNa
                    do
...
clean_dirs
# Gestion des droits sur l'application
please_wait "Manage Files and Folders Permissions"
set_rights
