...
# 19/04/2011 SCT : BZ 21888 => correction du paramètre de division par zéro (erreur SQL sur la division par zéro)
...
# 21/08/2012 BBX, BZ 28519 : compatibility with redhat 6.2
...
# GETTING POSTGRESQL LOGIN
postgresLogin="postgres"
if [ -e $rep_install'/php/xbdd.inc' ] ; then
	postgresLogin=$(cat ${rep_install}/php/xbdd.inc | grep "AUser" | awk -F '"' '{print $2}')
fi

# GETTING POSTGRESQL PASSWORD
postgresPassword=""
if [ -e $rep_install'/php/xbdd.inc' ] ; then
	postgresPassword=$(cat ${rep_install}/php/xbdd.inc | grep "APass" | awk -F '"' '{print $2}')
fi

# COPYING FILES AND INSTALLING PATCH
if [ -d $rep_install'/modules/divparzero/lib/' ] ; then

	# TESTING VERSION
	if [ ! -f $rep_install'/modules/divparzero/lib/divparzero.'$version'_'$archi'.so' ] ; then
		echo "Error : Postgresql version $version ($archi bits) is not currently supported"
		exit 127
	fi
	if [ $version = "8.2" ] ; then
	cp -f $rep_install'/modules/divparzero/lib/divparzero.'$version'_'$archi'.so' $repertoire_so'divparzero.so'
	fi
	env PGPASSWORD=$postgresPassword $psql_bin -U $postgresLogin $database -p $postgresqlPort -f $rep_install'/modules/divparzero/lib/uninstall_divparzero.sql'
	env PGPASSWORD=$postgresPassword $psql_bin -U $postgresLogin $database -p $postgresqlPort -f $rep_install'/modules/divparzero/lib/divparzero.sql'
else
	echo "Error : cannot find /modules/divparzero/lib/ directory"
	exit 127
fi
