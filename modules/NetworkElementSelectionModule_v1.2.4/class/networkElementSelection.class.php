...
/*
	22/06/2009 GHX
		- Ajout d'un paramètre pour la fonction setSaveFieldProperties() qui permet d'appeler une fonction JS quand on clique sur save
	23/06/2009 GHX
		- Ajout de la fonction setResetButtonProperties qui permet de spécifier une fonction JS a appeler lorsque l'on reset la sélection des éléments résaux
   06/06/2011 MMT DE 3rd Axis
 *    -  ajout paramètre d'initialisation htmlIdPrefix pour la fonction JS initNaSelection
*/
?>
<?php
/*
...
class networkElementSelection
{
	...
	* 	$saveFieldProperties['callbackFunctionJS']
	* @var array
	*/
	...
	/**
	* Contient le nom de la propriété name et id du champ input hidden dans lequel la sélection courante est sauvegardée.
	* 	$resetButtonProperties['id']
	* 	$resetButtonProperties['callbackFunctionJS']
	* @var array
	*/
	protected $resetButtonProperties;
	...
	*/
	protected $optionList = array();
        ...
        /**
	* Booléen permettant de savoir si on est en mode lecture seule ou non
	* @var boolean
	*/
	...
	*/
	...
	/**
	* setResetButtonProperties : permet de définir l'appel d'une fonction JS
	*
	* @param string $callbackFunctionJS nom d'une fonction JS qui est appelé lorsque l'on clique sur le bouton save (default null)
	*/
	public function setResetButtonProperties ( $callbackFunctionJS = "" )
	{
		$this->resetButtonProperties['callbackFunctionJS'] = $callbackFunctionJS;
	}
	...
	* @param boll $display 0 ou 1 si = 1, le champ caché qui permet de stocker les données est affiché.
	* @param string $callbackFunctionJS nom d'une fonction JS qui est appelé lorsque l'on clique sur le bouton save (default null)
	*/
	public function setSaveFieldProperties($id, $value, $separator, $display, $name="", $callbackFunctionJS = "")
	{
		...
		// 16:33 22/06/2009 GHX
		$this->saveFieldProperties['callbackFunctionJS'] = $callbackFunctionJS;
	}
	...
	*/
	public function setViewCurrentSelectionContentButtonProperties($url_to_get_selection = '')
	{
		...
		$this->generateViewCurrentSelBtn();
	}
	...
	*/
	...
	/**
	* generateViewCurrentSelBtn : permet de générer le code html de la zone options (icônes view current selection...)
	*
	* @return html
	*/
	private function generateViewCurrentSelBtn()
	{
		$html = '';
		$html = '
	...
	*/
	...
	{
		$html_button = '';
		// 16:34 22/06/2009 GHX
		// Ajout du nom de la fonction de callback
		$html_button = '
			<div class="buttonsNaSelection">
				<!-- Bouton Reset -->
				<input type="button" onclick="resetNeSelection(\''.$this->resetButtonProperties['callbackFunctionJS'].'\'); closeNeSelection();"
					value="Reset" class="buttonNaSelection"
					onmouseover="updateNeSelectionWindowStatus(\'Reset current selection\')"
					onmouseout="updateNeSelectionWindowStatus(\'\')"/>
				&nbsp;&nbsp;
				<!-- Bouton Save -->
				<input type="button" onclick="saveCurrentSelection(\''.$this->saveFieldProperties['callbackFunctionJS'].'\'); closeNeSelection();"
	...
	public function generateIHM()
	{
		...
		$html .= "<div id='".$this->htmlIdPrefix."window_select_na' style='width:400px; display:none;'>";
			...
			$html .= "
                ...
                // 25/05/2010 BBX
                // Message zone
                $html .= "<div id='".$this->htmlIdPrefix."msgZone'></div>";
	...
	*/
	private function initJavascriptValues()
	{
		...
		";
 		...
 		$html_input_hidden .= "
			<input type='hidden'
				id='".$this->htmlIdPrefix."readOnly'
				value='".($this->readOnly ? '1' : '0')."' />
		";
		...
		// Initilisation des variables javascript.
		?>
		<script type="text/javascript">
			// Initialise toutes les variables globales javascript.
			//06/06/2011 MMT DE 3rd Axis ajout paramètre d'initialisation htmlIdPrefix
			initNaSelection('<?=$this->htmlIdPrefix?>');
		</script>
		<?php
	}
	...
	*/
	public function addIcon($label, $css_class, $js_onclick)
	{
 	...
 	/**
	* setReadOnlyMode : permet de se positionner en mode lecture seule
	*
	* @param $readOnly boolean : mode lecture seule ou non (true = lecture seule)
	*/
        public function setReadOnlyMode($readOnly = true)
        {
