...
include_once('Header_word.php');
include_once('Footer_word.php');
include_once('Section.php');
include_once('Font.php');
include_once('Bordered.php');
include_once('ParFormat.php');
include_once('Table_word.php');
include_once('Cell.php');
include_once('Image_word.php');
include_once('BorderFormat.php');

/**
 * Class for creating rtf documents.
 * @version 0.2.1
 * @author Denis Slaveckij <info@phprtf.com>
 * @copyright  2007 Denis Slaveckij
 * @package Rtf
 */
class Rtf {
	...
	function Rtf() {	  
		...
		// $this->content = '{\sp{\sn fBehindDocument}{\sv 1}}';
	}  
	...
	*/  	
	...
	function &addHeader($type = 'all') {
	  	if (empty($this->oddEvenDifferent) && $type == 'all') {
		    $header = new Header_word($this, $type);
	...
	function &addFooter($type = 'all') {
	  	if (empty($this->oddEvenDifferent) && $type == 'all') {
		    $footer = new Footer_word($this, $type);
		...
		// $this->footers[$type]->writeText("<b>".$this->titlepg."</b>", new Font(),null);
		return $footer;		
	} 
	...
	function sendRtf($fileName = "simple") {	
		$this->prepare();
		// echo $this->content;
		// $content = preg_replace('{\sp{\sn fBehindText}{\sv 0}}','{\sp{\sn fBehindText}{\sv 1}}', $this->content);
