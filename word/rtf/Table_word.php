...
class Table_word {
	 ...
	 * @access public
	 */
	function Table_word(&$container, $alignment = 'left') {	    
	...
	*/
	 ...
	 * @param string $text Hyperlinks text If false, hyperlink is writen in previous paragraph format.
  	 ...
  	 * @access public	
	 */
	function writeToCellHyperLink($row, $column, $hyperlink, $text, &$font, &$parFormat) {	  
		if ($this->checkIfCellExists($row, $column)) {		
		  	$cell = &$this->getCell($row, $column);
	  		$cell->writeHyperLink($hyperlink, $text, $font, $parFormat);	  		
		}
	}
	...
	function setVerticalAlignmentOfCells($verticalAlignment, $startRow, $startColumn, $endRow = 0, $endColumn = 0) {		
		Table_word::rowsCols($startRow, $startColumn, $endRow, $endColumn);
