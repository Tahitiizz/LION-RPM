 ...
 * $Author: f.guillard $
 * $Date: 2014-09-25 12:07:49 +0200 (jeu., 25 sept. 2014) $
 * $Revision: 148778 $
 *
 * @version 5.1.0.03
 *
...
<? class RawModel extends RawKpiModel
{
	...
	public static $IFDOWN_COUNTER_OK = 0;
	public static $IFDOWN_COUNTER_KO_MIXED_KPI = 1;
	public static $IFDOWN_COUNTER_KO_QUERY_QB = 2;
	public static $IFDOWN_COUNTER_KO_GRAPH = 3;
	public static $IFDOWN_COUNTER_KO_ALARM = 4;
	public static $IFDOWN_COUNTER_KO_DATA_EXPORT = 5;
	...
	*/
    ...
    /**
     * Détermine si le compteur est désactivable
     * S'il ne l'est pas, renvoie le code d'erreur associé à la raison
     * 
     ...
     * 
     */
    public static function desactivateCounter($lineId, $family, $undeploy, $productId = 0) {
    	$result = array();
    	$result[0] = self::$IFDOWN_COUNTER_OK;
    	$database = DataBase::getConnection($productId);
		// On récupreè le nom de l'élément à mettre à jour.
		// 10:10 17/07/2009 GHX
		// Correction du BZ 10658 [REC][Mapping/counter activation]: désactivation compteur
		// On avait un point-virgule au lieu d'un point pour la concaténation de la chaine
		$query_name = "SELECT edw_target_field_name,edw_field_name,edw_field_name_label
                            FROM sys_field_reference WHERE id_ligne = '".$lineId."'";
		// echo "<br>" . $query_name . "<br>";
		$result_array_name = $database->getRow($query_name);
		$field_name = $result_array_name["edw_target_field_name"];
		$edw_field_name = $result_array_name["edw_field_name"];
		$lower_edw_field_name=strtolower($edw_field_name); //comme dans la formule des KPI on peut avoir les noms des compteurs qui n'ont pas la même casse que les compteurs, on va utiliser le compteur en minuscule
		$edw_field_label = $result_array_name["edw_field_label"];
		// echo "<br>Field name : " . $field_name . "<br>";
		// verification que le compteur à désactiver n'appartient pas à la formule d'un KPI actif. On met la formule en charactères minuscule pour comparer avec le compteur lui aussi en mode minuscule
		//MàJ 10/08/2007 - Jérémy : Bug 618 : Mofication de la requête de façon a aller vérifier par quelle famille est utilisé le ou les formules de KPI trouvées par la requête
		//			Si la famille $family n'utilise pas le compteur dans une formule de KPI on doit pouvoir la désactiver. Ajout de " AND t2.family='$family' "
		//MàJ 14/08/2007 - Jérémy : Bug 619 : Le filtre de la requête est affiné de façon à ne pas écarter des compteur qui ne sont utilisés dans aucun KPI
		//			Par exemple "Sess_drop" n'est utilisé par aucun KPI (lors de la détection du bug) mais puisque la chaine de caractère apparait dans "Sess_Drop_CN", il est tout de même écarté
		//			([^[:alnum:]_]|^) et ([^[:alnum:]_]|$)  sont utilisés respectivement avant et après la chaine de caractère
		//				[^[:alnum:]_]	permet d'exclure tous les caractères alphanumériques et le underscore
		//				^ et $ qui constituent la deuxième alternative permettent de gérer les cas où il n'y aurait rien avant et rien après la chaine de caractère
		//			L'opérateur LIKE a été remplacé par  " ~* " qui permet de tester les Expression rationnelle sous Postgresql
		$query_kpi = "
			SELECT DISTINCT t1.kpi_name
			FROM sys_definition_kpi t1, sys_definition_group_table t2
			WHERE t1.on_off=1
			AND t2.family='$family'
			AND t2.edw_group_table = t1.edw_group_table
			AND lower(kpi_formula) ~* '([^[:alnum:]_]|^)" . $lower_edw_field_name . "([^[:alnum:]_]|$)'";
		//__debug($query_kpi);
		$res_kpi = $database->execute($query_kpi);
		$result_nb_kpi = $database->getNumRows();

		// 16:12 13/10/2010 SCT : BZ 18427 => Désactivation de compteur utilisé pour la BH possible
		$resultatCounterLocked = self::isRawLockedBh($_GET['product'], $lower_edw_field_name, $family);

		// 14/08/2009 BBX : on empêche la désactivation d'un compteur utilisé dans un data_export
		$query_de = "SELECT DISTINCT export_id
			FROM sys_export_raw_kpi_data
			WHERE raw_kpi_id = '".$lineId."'";
        ...
        $query_alarme = "SELECT alarm_id
						FROM sys_definition_alarm_static
						WHERE
						( alarm_trigger_data_field = '".$edw_field_name."' AND alarm_trigger_type = 'raw' )
						OR
						( additional_field = '".$edw_field_name."' AND additional_field_type = 'raw' )
		
						UNION
						SELECT alarm_id
								FROM sys_definition_alarm_dynamic
                                WHERE
                                                ( alarm_field = '".$edw_field_name."' AND alarm_field_type = 'raw' )
                                                OR
								( alarm_trigger_data_field = '".$edw_field_name."' AND alarm_trigger_type = 'raw' )
                                                OR
								( additional_field = '".$edw_field_name."' AND additional_field_type = 'raw' )
                                ...
                                FROM sys_definition_alarm_top_worst
                                WHERE
                                                ( list_sort_field = '".$edw_field_name."' AND list_sort_field_type = 'raw' )
                                                OR
                                                ( additional_field = '".$edw_field_name."' AND additional_field_type = 'raw' )";
		$res_alarme = $database->execute($query_alarme);
		$result_nb_alarme = $database->getNumRows();
		
		// 03/02/2011 BBX
        // On empêche la désactivation d'un compteur utilisé dans le mixed kpi
        // BZ 20369
        if(ProductModel::getIdMixedKpi()) {
			$dbMk = DataBase::getConnection(ProductModel::getIdMixedKpi());
            $query = "	SELECT id_ligne FROM sys_field_reference
						WHERE old_id_ligne = '".$lineId."'
                        AND sfr_sdp_id = ".$_GET['product'];
			$dbMk->execute($query);
			if($dbMk->getNumRows() > 0) {
				// CAS 1 : compteur utilise dans le mixed kpi
				$result[0] = self::$IFDOWN_COUNTER_KO_MIXED_KPI;
				$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_7',$edw_field_name,$edw_field_name_label);
				return $result;
			}
		}
		// FIN BZ 20369
		...
		// 31/08/2011 BBX
		// On empêche la désactivation d'un compteur utilisé dans une query
		// BZ 23524
		$queryAllQueries = "SELECT u.login, r.texte, r.requete
			FROM report_builder_save r, users u
			WHERE r.id_user = u.id_user";
		// 28/11/2011 BBX
		// BZ 24878 : on évite d'écraser la variable $res...
        $resQ = $database->execute($queryAllQueries);
		while($row = $database->getQueryResults($resQ,1)) {
			$data = unserialize($row['requete']);
			foreach($data['donnees_hidden'] as $oneData) {
				if(trim($oneData) != '') {
					$allData = explode(':',$oneData);
					// 29/09/2011 BBX ajout strtolower BZ 23524
					if($allData[3] == 'raw' && strtolower($edw_field_name) == strtolower($allData[2])) {
						$counter = empty($edw_field_name_label) ? $edw_field_name : $edw_field_name_label;
						$result[0] = self::$IFDOWN_COUNTER_KO_QUERY_QB;
						$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_8',$counter,$row['texte'],$row['login']);
						return $result;
					}
				}
			}
		...
		// 23/03/2012 BBX
        // BZ 26044 : on empeche la désactivation d'un compteur utilise dans un graphe
		$rawModel = new RawModel();
		$graphList = $rawModel->getGraphListWith ( $lineId, $productId );
		if(!empty($graphList)) {
			$counter = empty($edw_field_name_label) ? $edw_field_name : $edw_field_name_label;
			$result[0] = self::$IFDOWN_COUNTER_KO_GRAPH;
			$result[1] = __T('A_MAPPING_COUNTERS_COUNTER_USED_IN_GRAPH',$counter);
			$result[2] = $graphList;
			return $result;
		}
		// FIN BZ 26044
		
		if($result_nb_alarme > 0) {
			$result[0] = self::$IFDOWN_COUNTER_KO_ALARM;
			if ($edw_field_name_label != "") {
				$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_5',$edw_field_name,$edw_field_name_label);
			} else {
				$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_6',$edw_field_name);
			...
			if($result_nb_de > 0) {
				...
				$result[0] = self::$IFDOWN_COUNTER_KO_DATA_EXPORT;
				if ($inHidden) {
					$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_4',$edw_field_name);
				} else {
					$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_3',$edw_field_name);
				}
			}
			else {
				// 16:12 13/10/2010 SCT : BZ 18427 => Désactivation de compteur utilisé pour la BH possible
				// DEBUT CORRECTION BZ 18427
				if(!$resultatCounterLocked) {
					//__debug($result_nb_kpi,"Nombre de ligne trouvé pour le compteur $lower_edw_field_name");
					if ( $result_nb_kpi == 0 ) {
						// on met la valeur à 0 pour tous les groupes table de la famille roaming qui sont visibles.
						$query_groupe = "SELECT id_ligne FROM sys_definition_group_table
							WHERE visible=1 AND family='$family' ";
		
						// 30/07/2009 GHX : Correction d'un problème quand on désactive plusieurs compteurs en même temps
						// 10/03/2011 OJT : Correction bz20811, nouvelle gestion de la désactivation des compteurs
						$id_groupe = $database->getOne( $query_groupe );
	
						// 16/08/2011 BBX
						// Utilisation de l'id_ligne au lieu du nom du compteur pour l'identifier
						// BZ 23295
						if( !$undeploy ) {
							// On ne force pas la suppression du compteur, la desactivation suffit
							$query = "UPDATE sys_field_reference SET on_off=0 WHERE id_ligne='".$lineId."' AND id_group_table={$id_groupe}";
						}
						else {
							// On force la suppression du compteur.
							$query = "UPDATE sys_field_reference SET new_field=2 WHERE id_ligne='".$lineId."' AND id_group_table={$id_groupe}";
						}
						$database->execute( $query );
					}
					else {
						$result[0] = self::$IFDOWN_COUNTER_KO_DATA_EXPORT;
						if ($edw_field_name_label != "") {
							$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_1',$edw_field_name,$edw_field_name_label);
						} else {
							$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_2',$edw_field_name);
						}
					}
				}
				else {
					$result[0] = self::$IFDOWN_COUNTER_KO_DATA_EXPORT;
					$result[1] = __T('A_MAPPING_COUNTERS_ACTIVATION_COUNTER_DEACTIVATION_DISABLED_5',$edw_field_name);
				}
				// FIN CORRECTION BZ 18427
			}
		}
		return $result;
	}
