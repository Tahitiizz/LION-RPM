 ...
 * 22/04/2013 GFS - Bug 33065 - [SUP][TA Gateway][AVP 33610][Telus]: T&A Gateway freeze when a filter is apply on some parent Net Element
*/
?>
<?php
/**
	...
	************************************************************************/
	...
	public function getNaPaths($axe = 1, $productTable = array())
	{
		if (empty($productTable)) {
			$productTable = self::getDashboardProducts($this->idDashboard);
		}
		...
		foreach ($na_paths as $family => $family_paths) {
			...
			if(array_key_exists($axe, $family_paths)){
				...
				foreach ($paths_axe as $na_master => $na_child) {
					...
					// 22/04/2013 GFS - Bug 33065 - [SUP][TA Gateway][AVP 33610][Telus]: T&A Gateway freeze when a filter is apply on some parent Net Element 
					$na_child_tmp = array_intersect(array_unique($na_child), $na_levels);
					foreach ($na_child_tmp as $child) {
						if (in_array($na_master, $na_levels) && in_array($child, $na_levels))
						{
							// 09:07 30/07/2009 GHX
							// Correction du BZ 10439
							// Met les éléments fils dans un tableau dans le cas où on a 2 fils différents
							$paths[$na_master][]  = $child;
						}
					}
				}
			}
