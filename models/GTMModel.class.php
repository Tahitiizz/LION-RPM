 ...
 * Since CB 5.1.6.21
 * 
 * 07/06/2012 NSE bz 27406 : pb lorsque id_product > 9
	...
	function getNALevelsInCommon($axe = 1)
	{
		...
		foreach ($this->getGTMProducts() as $p) {
			...
			foreach ($na_labels as $key => $val) {
                            // 07/06/2012 NSE bz 27406 : $p est directement l'id_product et non un tableau
                            $all_na_labels[$p."_".$key] = $val;
			}
		}		
	...
	************************************************************************/
	...
	public function getGTMGisInformations()
	{
		...
		if (count($gis_infos_tmp) > 0 && $gisActived) {
			...
			// 09/10/2012 ACS DE GIS 3D ONLY
			return array('id' => $gis_infos['id_ligne'], 'name' => $gis_infos['name'], 'label' => '', 'type' => $gis_type, 'family' => $gis_infos['family'], 'product' => $gis_infos_tmp['id_product'], 'gis_mode' => $gisActived);
		}
		else {
			return array();
		}
	...
	public function getInfoLinkToAA ()
	{
        ...
        public function getInfoLinkToNE ()
	{
		$linkToNE = array('raw' => array(), 'kpi' => array());
		...
		$queryInfoLinkToNE = "
				...
				WHERE 
					snefk_family = '%s'
					AND snefk_type = '%s'
					AND lower(edw_field_name) = lower(snefk_idkpi) 
					AND lower(edw_field_name) IN (%s)
			";
		...
		if ( count($rawsByProduct) > 0 )
		{
			foreach ( $rawsByProduct as $idProduct => $rawsByFamily )
			{
				...
				foreach ( $rawsByFamily as $family => $raws )
				{
					$resultInfoLinkToNE = $db->execute(sprintf($queryInfoLinkToNE,
							$family,
							'raw',
							implode(", ", array_map(array($this, 'labelizeValue'), array_map('strtolower',array_keys($raws))))
						));
					...
					if ( $db->getNumRows() > 0 )
					{
						while ( $row = $db->getQueryResults($resultInfoLinkToNE, 1) )
						{
							$linkToNE['raw'][$row['id_ligne']] = array('idProduct' => $idProduct, 'family' => $family, 'labelNE' => $row['snefk_label_link'], 'snefk_idkpi' => $row['snefk_idkpi']);
						}
					}
				}
			}
		...
		$queryInfoLinkToNE = "
				...
				WHERE 
					snefk_family = '%s'
					AND snefk_type = '%s'
					AND lower(kpi_name) = lower(snefk_idkpi) 
					AND lower(kpi_name) IN (%s)
			";
			...
			foreach ( $kpisByProduct as $idProduct => $kpisByFamily )
			{
				...
				foreach ( $kpisByFamily as $family => $kpis )
				{
					...
					if ( $db->getNumRows() > 0 )
					{
						while ( $row = $db->getQueryResults($resultInfoLinkToNE, 1) )
						{
							$linkToNE['kpi'][$row['id_ligne']] = array('idProduct' => $idProduct, 'family' => $family, 'labelNE' => $row['snefk_label_link'], 'snefk_idkpi' => $row['snefk_idkpi']);
						}
					}
				}
			}
