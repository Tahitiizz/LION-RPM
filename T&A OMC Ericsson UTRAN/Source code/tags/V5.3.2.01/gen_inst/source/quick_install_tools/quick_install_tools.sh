...
exit_if_active_counters_not_deployed() {
	[ -d "/home/$appname" ] || return # ce n'est pas un cas de migration ? alors rien a verifier
	nb_counter=$($psql_bin -U postgres -d $dbname -p $postgresqlPort -tAc "SELECT count(edw_field_name) FROM sys_field_reference WHERE on_off = 1 AND new_field = 1" 2>>$LOG)
	if [ "$nb_counter" -gt 0 ]
	then
		cat <<EOF
...
sysfieldreference() {
	if [ "$1" = backup ] ; then
		$psql_bin -U postgres -d $dbname -p $postgresqlPort -tAc "SELECT edw_field_name, edw_group_table , on_off FROM sys_field_reference" > "$sfr_bkp_file" 2>>$LOG
	elif [ "$1" = restore ] ; then
